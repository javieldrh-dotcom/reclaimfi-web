...
export default function SalesBookPage() {
  ...
  async function reverseEntry(entryId: string, journalEntryId: string, customerNameLocal: string) {
    ...
    await supabase.from("sales_book_entries").update({ is_reversed: true }).eq("id", entryId);
              ...
              <tbody>
                {entries.slice(0, displayLimit).map((e) => (
                  <tr key={e.id} style={{ borderBottom: "1px solid #1F2937" }}>
                    ...
                    <td style={{ padding: 6 }}>{e.customer_name}{e.is_reversed && <span style={{ marginLeft: 6, fontSize: 11, color: "#FB923C", border: "1px solid #FB923C", borderRadius: 4, padding: "1px 5px" }}>REVERSADO</span>}</td>
