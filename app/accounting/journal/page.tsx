...
export default function JournalPage() {
  ...
  async function saveEntry() {
    ...
    const { error: e2 } = await supabase.from("journal_lines").insert(rows);
    if (e2) { setMessage("Error: " + e2.message); return; }
    setMessage("Asiento NÃ‚Âº " + nextNumber + " guardado correctamente.");
    setDescription("");
  ...
  async function reverseEntry(entry: any) {
    ...
    const confirmMsg = "Se creara un asiento nuevo con las cifras invertidas de NÃ‚Âº" + (entry.entry_number ?? "S/N") + ". El asiento original permanecera visible. Confirmar?";
    ...
    const { data: newEntry, error: entryError } = await supabase.from("journal_entries").insert([{
      ...
      description: "Reverso del Asiento NÃ‚Âº" + (entry.entry_number ?? "S/N") + " - " + entry.description,
      entry_date: today,
  ...
  return (
    <VerticalPageLayout
      vertical="accounting"
      title="Libro Diario"
      subtitle={lastEntryNumber > 0 ? "Ultimo asiento del ejercicio " + selectedYear + ": NÃ‚Âº " + lastEntryNumber : undefined}
      fullWidth
            ...
            <div key={e.id} style={{ ...theme.cardStyle, marginTop: 12, opacity: e.status === "VOIDED" ? 0.5 : 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 700, fontSize: 22 }}>
                  NÃ‚Âº{e.entry_number ?? "S/N"} - {e.entry_date} - {e.description}
              ...
              {(e.journal_lines ?? []).map((l: any, idx: number) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 20, color: "#B0B8C8", marginTop: 6, paddingLeft: 12 }}>
                  <span>{presentationMode ? l.chart_of_accounts?.account_name : "Fol." + (l.chart_of_accounts?.mayor_folio ?? "-") + " Ã‚Â· " + l.chart_of_accounts?.account_code + " - " + l.chart_of_accounts?.account_name}</span>
