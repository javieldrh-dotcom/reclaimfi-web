    ...
    <div style={{ background: "radial-gradient(ellipse 90% 50% at 50% -10%, " + bgCard + " 0%, " + bg + " 60%, #040911 100%)", minHeight: "100vh", color: ink, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "22px 48px", borderBottom: "1px solid " + cardBorder, flexWrap: "wrap", gap: 16, background: "rgba(10,22,40,0.85)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 22, flexWrap: "wrap" }}>
          ...
          <Link href={"/" + locale + "/soluciones"} style={{ color: sky, textDecoration: "none", fontSize: 17, fontWeight: 700 }}>{tNav("soluciones")}</Link>
      ...
      <section style={{ padding: "20px 40px 90px", maxWidth: 1150, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
          <div style={cardStyle}>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: sky }}>{t("role1Title")}</h2>
            <p style={{ marginTop: 14, fontSize: 18, color: inkSoft, lineHeight: 1.7 }}>{t("role1Desc")}</p>
          </div>
          <div style={cardStyle}>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: sky }}>{t("role2Title")}</h2>
            <p style={{ marginTop: 14, fontSize: 18, color: inkSoft, lineHeight: 1.7 }}>{t("role2Desc")}</p>
          </div>
          <div style={cardStyle}>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: sky }}>{t("role3Title")}</h2>
            <p style={{ marginTop: 14, fontSize: 18, color: inkSoft, lineHeight: 1.7 }}>{t("role3Desc")}</p>
          </div>
          <div style={cardStyle}>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: sky }}>{t("role4Title")}</h2>
            <p style={{ marginTop: 14, fontSize: 18, color: inkSoft, lineHeight: 1.7 }}>{t("role4Desc")}</p>
          </div>
        </div>
      </section>
